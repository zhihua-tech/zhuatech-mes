...
新增 `POST /api/enterprise/mes/production-genealogy`，在成品批次入账前核对工单、BOM、工艺路线、设备、操作员、质量放行和电子签名，并逐项校验投入物料批次的质量状态、有效期、工单发料、追溯码、BOM 超耗及关键物料供应商批次。接口返回 `RECORD / REVIEW / BLOCKED`，可作为报工入库和 WMS 标签同步的前置门禁。
